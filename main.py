...
async def get_steam_ranks(country: str, start: int, end: int, db: AsyncSession = Depends(get_rdb)):
    ...
    query = text("SELECT g.game_id """"game_id"""", g.game_name """"game_name"""", g.header_image_url """"header_image_url"""", p.price """"price"""" FROM games g JOIN game_prices p ON g.game_id = p.game_id WHERE p.currency = 'KRW' AND g.game_id IN :ids")
